...
void fft_initialize( FFT_Tables *fft_tables )
{
	...
	fft_tables->reordertbl	= (short unsigned int **)AllocMemory( (MAXLOGM+1) * sizeof( fft_tables->reordertbl[0] ) );
...
static void reorder( FFT_Tables *fft_tables, double *x, int logm)
{
	...
	{
		fft_tables->reordertbl[logm] = (short unsigned int *)AllocMemory(size * sizeof(*(fft_tables->reordertbl[0])));
