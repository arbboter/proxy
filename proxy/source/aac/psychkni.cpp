			    ...
			    int *cb_width_short, int num_cb_short)
{
  ...
  for (win = 0; win < 8; win++)
  {
	...
	/*memcpy(transBuffS, psyInfo->prevSamplesS,psyInfo->sizeS * sizeof(double));
	memcpy(transBuffS + psyInfo->sizeS, newSamples + win*psyInfo->sizeS,psyInfo->sizeS * sizeof(double));*/
 ...
 //     if (first >= psydata->bandS) // band out of range
	...
	//memcpy(psyInfo->prevSamplesS, newSamples + win*psyInfo->sizeS,psyInfo->sizeS * sizeof(double));
  }
