...
/*
	__DEBUG_NEW__ 宏 检查内存申请释放情况
	[2013-07-19 L]
*/

#ifdef __DEBUG_NEW__
void * test_faac_malloc(unsigned int size, const char *file, int line);
void test_faac_free(void * ptr);

/* Memory functions */
#define AllocMemory(SIZE_MEM) test_faac_malloc(SIZE_MEM, __FILE__, __LINE__)
#define FreeMemory(PTR_MEM) test_faac_free(PTR_MEM)
